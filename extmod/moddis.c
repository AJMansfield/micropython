        ...
        if (frozen_type == MP_FROZEN_MPY) {
            ...
            if (mp_proto_fun_is_bytecode(frozen->proto_fun)) {
                ...
                *cm.context = *obj_fun_bc->context;
                return (mp_compiled_module_t) {
                           cm.context, obj_fun_bc->rc,
                           #if MICROPY_PERSISTENT_CODE_SAVE
                           cm.has_native, cm.n_qstr, cm.n_obj,
                           #endif
                };
            } else {
                return (mp_compiled_module_t) {
                           cm.context, (mp_raw_code_t *)frozen->proto_fun,
                           #if MICROPY_PERSISTENT_CODE_SAVE
                           cm.has_native, cm.n_qstr, cm.n_obj,
                           #endif
                };
...
const mp_print_t *get_print_for_file(mp_obj_t file) {
    const mp_print_t *print = MP_PYTHON_PRINTER;
    #if MICROPY_PY_IO
    mp_print_t print_data;
    if (file != mp_const_none) {
        mp_get_stream_raise(file, MP_STREAM_OP_WRITE);
        print_data = (mp_print_t) {file, mp_stream_write_adaptor};
        print = &print_data;
    }
    #endif
    return print;
